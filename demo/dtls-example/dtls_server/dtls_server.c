...
//	SSL_CTX_set_verify_depth (ctx, 2);
	...
	/* Set DTLS cookie generation and verification callbacks */
        SSL_CTX_set_cookie_generate_cb(ctx, dtls_generate_cookie_callback);
        SSL_CTX_set_cookie_verify_cb(ctx, dtls_verify_cookie_callback);
...
static int dtls_build_server_cert (void)
{
	...
	unlink ("serial");
	unlink ("index.txt");
	snprintf (GEN, BUFSIZ, "openssl req -nodes -x509 -days 365 -newkey rsa:2048 -keyout %s -out %s -config "
	...
	snprintf (GEN, BUFSIZ, "openssl req -nodes -new -newkey rsa:2048 -config tempsrv.cnf -keyout %s -out %s", \
	...
	snprintf (GEN, BUFSIZ, "openssl dhparam -out %s 2048", DTLS_DH_FILE);
	system (GEN);
...
static int dtls_build_client_cert (void)
{
	...
	memset (GEN,0, sizeof (GEN));
	snprintf (GEN, BUFSIZ, "openssl req -nodes -new -newkey rsa:2048 -config tempcli.cnf -keyout %s -out %s", \
