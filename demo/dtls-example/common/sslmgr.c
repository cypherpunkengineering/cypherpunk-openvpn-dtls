...
int dtls_generate_cookie_callback(SSL *ssl, unsigned char *cookie, unsigned int *cookie_len)
{
	unsigned char *buffer, result[EVP_MAX_MD_SIZE];
        ...
        if (!cookie_initialized)
                ...
                if (!RAND_bytes(cookie_secret, COOKIE_SECRET_LENGTH))
                        {
                        dtls_report_err("error setting random cookie secret\n");
                        return 0;
                        }
...
int dtls_verify_cookie_callback(SSL *ssl, unsigned char *cookie, unsigned int cookie_len)
{
        unsigned char *buffer, result[EVP_MAX_MD_SIZE];
        unsigned int length, resultlength;
        struct sockaddr_in peer;
